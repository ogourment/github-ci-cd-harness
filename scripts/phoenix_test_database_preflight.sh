...
linked_worktree() {
  if [[ "$worktree_override" == "1" ]]; then return 0; fi
  if [[ "$worktree_override" == "0" ]]; then return 1; fi
...
[[ -n "${MIX_TEST_PARTITION:-}" ]] || exit 0

mix_bin="${PHOENIX_TEST_DATABASE_MIX_BIN:-mix}"
MIX_ENV=test "$mix_bin" ecto.create --quiet && exit 0
...
printf '%s\n' 'The creation command must run with MIX_ENV=test; plain `mix ecto.create` targets the default environment.' >&2
exit 2
