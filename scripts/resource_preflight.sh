...
[[ -r "$meminfo_path" ]] || exit 0

read_kib() { awk -v field="${1}:" '$1 == field {print $2; exit}' "$meminfo_path"; }
...
if [[ ! "$mem_total_kib" =~ ^[0-9]+$ || ! "$mem_available_kib" =~ ^[0-9]+$ ||
      ! "$swap_total_kib" =~ ^[0-9]+$ || ! "$swap_free_kib" =~ ^[0-9]+$ || "$mem_total_kib" -eq 0 ]]; then
  printf '%s\n' "RESOURCE WARNING: could not parse memory data from $meminfo_path; continuing." >&2
  exit 0
fi

min_value() { if (( $1 < $2 )); then printf '%s' "$1"; else printf '%s' "$2"; fi; }
swap_used_kib=$((swap_total_kib > swap_free_kib ? swap_total_kib - swap_free_kib : 0))
if (( swap_total_kib > 0 )); then swap_percent=$((swap_used_kib * 100 / swap_total_kib)); else swap_percent=0; fi
...
read_vmstat() { [[ -r "$2" ]] && awk -v field="$1" '$1 == field {print $2; exit}' "$2"; }
psi_some_avg10=0; psi_full_avg10=0
if [[ -r "$psi_path" ]]; then
...
swap_in_delta=0; swap_out_delta=0
if [[ "$swap_in_before" =~ ^[0-9]+$ && "$swap_out_before" =~ ^[0-9]+$ ]]; then
  if [[ "$vmstat_after_path" == "$vmstat_path" && "$sample_seconds" != "0" ]]; then sleep "$sample_seconds"; fi
...
float_ge() { awk -v actual="$1" -v threshold="$2" 'BEGIN {exit !(actual >= threshold)}'; }

pressure_active=false; pressure_severe=false
if float_ge "$psi_some_avg10" "${RESOURCE_PREFLIGHT_PSI_WARNING:-1.0}" ||
   (( swap_activity_pages >= ${RESOURCE_PREFLIGHT_SWAP_ACTIVITY_WARNING_PAGES:-64} )); then pressure_active=true; fi
if float_ge "$psi_full_avg10" "${RESOURCE_PREFLIGHT_PSI_CRITICAL:-0.5}" ||
   (( swap_activity_pages >= ${RESOURCE_PREFLIGHT_SWAP_ACTIVITY_CRITICAL_PAGES:-1024} )); then pressure_severe=true; fi
...
if (( mem_available_kib <= critical_available_kib )) ||
  ...
  status=critical
elif (( mem_available_kib <= warning_available_kib )) || [[ "$pressure_active" == true ]]; then status=warning; fi
